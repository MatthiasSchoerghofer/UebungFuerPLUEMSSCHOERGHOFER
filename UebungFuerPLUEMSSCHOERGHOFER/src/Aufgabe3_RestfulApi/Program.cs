...
public class Program
{
    ...
    public static void Main(string[] args)
    {
            ...
            CancellationToken cancellationToken) =>
        {
            ...
            try
            {
                return Results.Ok(await service.UpdateSongAsync(id, cmd, cancellationToken));
            }
            catch (NotFoundException ex)
            {
                return Results.NotFound(new ProblemDetails { Detail = ex.Message });
            ...
            CancellationToken cancellationToken) =>
        {
            try
            {
                return Results.Ok(await service.SearchSongsAsync(
                    term,
                    genre,
                    minStreams,
                    maxDurationSeconds,
        ...
        api.MapGet("/artists/statistics", async (
            ...
            CancellationToken cancellationToken) =>
        {
            try
            {
                return Results.Ok(await service.GetArtistStatisticsAsync(minSongs ?? 1, cancellationToken));
            }
            catch (ArgumentException ex)
            {
                return Results.BadRequest(new ProblemDetails { Detail = ex.Message });
        ...
        api.MapGet("/genres/summary", async (ISongService service, CancellationToken cancellationToken) =>
            Results.Ok(await service.GetGenreSummariesAsync(cancellationToken)));
